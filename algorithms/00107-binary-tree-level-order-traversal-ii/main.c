...
int** levelOrderBottom(struct TreeNode* root, int* returnSize, int** returnColumnSizes){
    ...
    int *returnNums_tmp,returnCol_tmp;
    for(i=0,j=returnLen-1;i<j;i++,j--){
        returnNums_tmp = returnNums[i];
        returnNums[i] = returnNums[j];
        returnNums[j] = returnNums_tmp;

        returnCol_tmp = returnCol[i];
        returnCol[i] = returnCol[j];
        returnCol[j] = returnCol_tmp;
    }
...
int main(){
    ...
    returnNums = levelOrderBottom(&root[0],&returnSize,&returnColumnSizes);
    int i,j;
